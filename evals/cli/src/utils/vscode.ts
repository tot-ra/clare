...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	const settings = {
		...
		"workbench.view.extension.tot-ra.clarifai-agent-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.tot-ra.clarifai-agent-ActivityBar",
			when: "viewContainer.workbench.view.extension.tot-ra.clarifai-agent-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const args = [
		...
		"--command",
		"workbench.view.extension.tot-ra.clarifai-agent-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.tot-ra.clarifai-agent-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						await vscode.commands.executeCommand('workbench.view.extension.tot-ra.clarifai-agent-ActivityBar');
