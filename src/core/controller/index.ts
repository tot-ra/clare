		...
		switch (message.type) {
			...
			case "accountLoginClicked": {
				...
				const authUrl = vscode.Uri.parse(
					`https://app.cline.bot/auth?state=${encodeURIComponent(nonce)}&callback_url=${encodeURIComponent(`${uriScheme || "vscode"}://tot-ra.clare/auth`)}`,
				)
			...
			case "openExtensionSettings": {
				...
				await vscode.commands.executeCommand(
					...
					`@ext:tot-ra.clare ${settingsFilter}`.trim(), // trim whitespace if no settings filter
				)
				break
			}
			case "invoke": {
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
	...
	async addSelectedCodeToChat(code: string, filePath: string, languageId: string, diagnostics?: vscode.Diagnostic[]) {
		...
		await vscode.commands.executeCommand("clare.SidebarProvider.focus")
